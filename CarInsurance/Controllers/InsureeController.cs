    ...
    public class InsureeController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,FirstName,LastName,EmailAddress,DateOfBirth,CarYear,CarMake,CarModel,DUI,SpeedingTickets,CoverageType,Quote")] Insuree insuree)
        {
            if (ModelState.IsValid)
            {
                ...
                int porcheAdd3 = porche_Two ? 25 : 0;////works - gets factored into beginning sum 'tertTotal'



                var ticketsInput = insuree.SpeedingTickets;

                int ticketsVerify = ticketsInput<0 ? 0 : insuree.SpeedingTickets;


                int tickets = ticketsVerify * 10;////works - gets factored into beginning sum 'tertTotal'

                decimal tertTotal = 50 + appAgeAdd1 + appAgeAdd2 + appAgeAdd3 + vehAgeAdd1 + vehAgeAdd2 + porcheAdd1 + porcheAdd2 + porcheAdd3 + tickets;





                decimal tertTotal2 = 1.25m * tertTotal; // use of double and later converting back to int


                bool addTest = insuree.DUI;

                decimal sQuote = addTest? tertTotal2 : tertTotal;



                decimal secTotal = 1.5m * sQuote; 

                bool addTest2 = insuree.CoverageType;

                decimal ssecTotal2 = insuree.CoverageType ? secTotal : sQuote;

                decimal primTotal = ssecTotal2;
